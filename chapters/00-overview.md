...
**如果你关心...**
- **UI 实现**: 阅读 **01-ink-architecture.md** (Ink 终端 UI)
- **权限系统**: 阅读 **02-permission-system.md** (权限管理)
- **MCP 协议**: 阅读 **03-mcp-integration.md** (MCP 集成)
- **工具系统**: 阅读 **04-tool-system.md** (Tool 工具系统)
- **状态管理**: 阅读 **07-state-management.md** (状态管理)
- **长时间运行**: 阅读 **08-task-framework.md** (任务框架)

**学习路径建议**:
1. **入门**: 00 → 01 → 04 (架构总览 → UI → 工具系统)
2. **进阶**: 02 → 03 → 07 (权限 → MCP → 状态管理)
3. **高级**: 06 → 08 → 09 (查询引擎 → 任务框架 → 多 Agent)
